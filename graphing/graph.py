...
def create_pred_graph(dff, p1_player_name, p2_player_name):
    ...
    data = [
        go.Scatter(x=dff['set_time'], y=current_set_pred_smooth, xaxis='x', yaxis='y2', name=p1_player_name, mode='lines', customdata=cols['p1'], legendgroup=p1_player_name, showlegend=False, line=dict(color=PLAYER_COLOURS['p1'])),
    ...
    for p2_round_win_time in final_round_times.loc[final_round_times['p1_round_win']==False,'set_time'].values:
        fig.add_vline(x=p2_round_win_time, line_width=2, line_color=PLAYER_COLOURS['p2'], annotation_text=f'{p2_player_name} wins round', annotation_position="top right", row='all',col='all')
        fig.add_layout_image(dict(xref="x", yref="paper", sizex=0.1, sizey=0.1))
    for p1_round_win_time in final_round_times.loc[final_round_times['p1_round_win']==True,'set_time'].values:
        fig.add_vline(x=p1_round_win_time, line_width=2, line_color=PLAYER_COLOURS['p1'], annotation_text=f'{p1_player_name} wins round', annotation_position="top right", col='all')
...
def create_match_stats_graph(match_stats_dff, p1_player_name, p2_player_name, p1_set_win, p1_round_win, stat_col_name, graph_title):
    ...
    fig.update_traces(marker=dict(colors=[PLAYER_COLOURS['p2'], PLAYER_COLOURS['p1']]), textposition='inside', hoverinfo='label+value', textinfo='label+value')
